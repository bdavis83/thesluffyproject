...
const EpicPass = () => {
  const [epicResorts, setEpicResorts] = useState([]);
    ...
    fetch("http://127.0.0.1:8000/api/ski_resorts/all/")
      ...
      .then((data) => {
        const filteredResorts = data.filter(
          (resort) => resort.pass_type === "Epic"
        );
        setEpicResorts(filteredResorts);
      })
  ...
  return (
    <div>
      <h1>Epic Pass Resorts</h1>
      <div className="resort-cards-container">
        {epicResorts.map((resort) => (
